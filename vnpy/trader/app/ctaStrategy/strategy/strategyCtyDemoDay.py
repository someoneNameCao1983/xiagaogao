...
########################################################################
class CtyEmaDemoStrategyDay(CtaTemplate):
    """双指数均线策略Demo"""
    className = 'CtyEmaDemoStrategyDay'
    ...
    slowHistory = []              # 长均线数组
    holdTimes = []                # 持仓时间
    minHistory = 21               # 最少缓存60根开始计算
    maxHistory = 44               # 最多缓存120根
    ...
    def __init__(self, ctaEngine, setting):
        """Constructor"""
        super(CtyEmaDemoStrategyDay, self).__init__(ctaEngine, setting)
    ...
    def onBar(self, bar):
        ...
        # 计算快慢均线
        fastSMA = ta.SMA(closeArray, timeperiod=self.cycle)
        self.fastMa0 = fastSMA[-1]
        self.fastMa1 = fastSMA[-2]
        barATR = ta.ATR(highArray[self.minHistory*-1:-1], lowArray[self.minHistory*-1:-1], closeArray[self.minHistory*-1:-1])
        self.shortATR = barATR[-1]
        #closeArray.sort()
        #除噪音 30天最高
        #self.lowPrice = (closeArray[0]+closeArray[1])/2
        #self.highPrice = (closeArray[-1]+closeArray[-2])/
        self.lowPrice = ta.MIN(closeArray, timeperiod=self.cycle)[-1]
        self.highPrice = ta.MAX(closeArray, timeperiod=self.cycle)[-1]
    ...
    def monitorQuote(self):
        ...
        if float(self.bar.close) == self.lowPrice:
            ...
            posleve = abs(self.highPrice - float(self.bar.close)) / abs(float(self.bar.close) - self.lowPrice)
        if posleve < 1/9:
            self.kcFlag = True
    ...
    def kcATR(self):
        ...
        #if self.highPrice - self.lowPrice > 2*self.shortATR and self.bar.close >= float(self.bar.open) + self.shortATR:
        if self.highPrice - self.lowPrice > 2*self.shortATR and self.bar.close >= self.highPrice:
            openFlag = True
        '''
    ...
    def monitorPOS(self):
        ...
        # 趋势分数
        barScore = 0
        if fastSMA[-6] > fastSMA[-1]:
            barScore = barScore - 10
        if fastSMA[-6] < fastSMA[-1]:
            barScore = barScore + 10
        # 盈利分数
        #diff = fastSMA[-1] - self.openPrice
        #rat = diff / self.openPrice * 100 * 10
        #barScore = barScore + diff1/10
        diff = float(self.bar.close) - self.openPrice
        barScore = barScore + diff
        ...
        if float(self.bar.close) == self.lowPrice:
            ...
            posleve = abs(self.highPrice - float(self.bar.close))/abs(float(self.bar.close) - self.lowPrice)
        # 价格超过开仓支撑位分数
        if self.bar.close > self.topPrice:
            barScore = barScore + 10
        if self.highPrice-self.lowPrice > 2*self.shortATR:
            if posleve <= 1/19:
        ...
        if self.holdScore - barScore > 30 + self.shortATR:
            closeFlag = True
            print ("昨分:%f,今分：%f" % (self.holdScore, barScore))
        self.holdScore = barScore
        if self.holdScore < 0:
            closeFlag = True
        if closeFlag and self.holdBars > 1:
            print ("平仓:%s,位置:%f,分数：%f,价差:%f,持仓周期:%d" % (
                self.bar.datetime.strftime('%Y%m%d-%H%M'), posleve, self.holdScore, diff, self.holdBars))
